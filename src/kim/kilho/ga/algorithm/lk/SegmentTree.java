...
public class SegmentTree {
  ...
  private TwoEdgeTour _tour;  // The original tour for the segment tree
  private int _n;   // The number of cities in the tour
  private SEGMENT[] _seg_tree;  // Segment tree array
  ...
  public void setupTree(int to, int[] t) {
    ...
    // Every location pointers must be updated.
    ...
    if (to <= 2) {
      ...
      if ((_city_order[t[1]]+1)%_n == _city_order[t[2]])
        _seg_tree[0].reversal = false;
      else
        _seg_tree[0].reversal = true;
      _seg_size = 1;
    }
    if (to == 2) {
      // assert(_seg_size == 1);
...
//      }
  ...
  private void splitSegment(int c1, int c2) throws CloneNotSupportedException {
    ...
    seg = (SEGMENT)s1.clone();   // copy s1 to seg
  ...
  private boolean isCorrect() {
    ...
    while (current.idx_seg_tree != start.idx_seg_tree) {
      num_segment++;
  ...
  public int getNext(int city) {
    ...
    if (!a.reversal) {
      if (a_order < a.last) b_order = a_order + 1;
        ...
        b = a.next;
        b_order = (!b.reversal) ? b.first : b.last;
      }
    } else {
  ...
  public int getPrev(int city) {
    ...
    if (!a.reversal) {
      if (a_order > a.first) b_order = a_order - 1;
        ...
        b = a.prev;
        b_order = (!b.reversal) ? b.last : b.first;
      }
    } else {
      if (a_order < a.last) b_order = a_order + 1;
      else if (a_order == a.last) {
  ...
  public boolean isBetween(int start, int middle, int end) {
    ...
    if (ss.idx_seg_tree == sm.idx_seg_tree && ss.idx_seg_tree == se.idx_seg_tree) {
    ...
    } else if (ss.idx_seg_tree == se.idx_seg_tree)
      res = (ss.reversal) ? (starto < endo) : (endo < starto);
    else if (ss.idx_seg_tree == sm.idx_seg_tree)
      res = (ss.reversal) ? (mido < starto) : (starto < mido);
    else if (se.idx_seg_tree == sm.idx_seg_tree)
      res = (sm.reversal) ? (endo < mido) : (mido < endo);
    // Now, ss != sm != se, thus calculate using segments.
    else {
      cur = ss;
      while ((cur = cur.next).idx_seg_tree != se.idx_seg_tree)
        if (cur.idx_seg_tree == sm.idx_seg_tree) break;
      res = (cur.idx_seg_tree == sm.idx_seg_tree);
    }
    return res;
  }
}
